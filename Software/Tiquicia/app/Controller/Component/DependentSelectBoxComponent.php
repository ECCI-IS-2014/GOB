...
class DependentSelectBoxComponent extends Component {
		...
		public function newSimpleSelect($selectConfig=null,$thisRequest=null,$ajax,$currentModel=null){
			...
			if(!$thisRequest){
				...
				//echo '-------------------------------------- 1 ----------------------------------';
				// carga el selector con la data inicial. El primer selector o selecor padre
			...
			if($ajax){
				...
				if($thisRequest){
				//echo '-------------------------------------- 2 ----------------------------------';
				// actualiza el selector dependiente	
					...
					if($selectConfig[key($requestQuery)]['childClass']){
						...
						$data[Inflector::camelize($targetField)] = $this->$childClass->find('list', array('fields'=>$fields,'conditions' => $conditions));
					
						# lleno el array con los select sub-dependientes - nulos.
						
						if(isset($selectConfig[$childClass]['update'])){
							foreach($selectConfig[$childClass]['update'] as $v){
									
								$targetField = strtolower($v).'_id';	
								$data[Inflector::camelize($targetField)] = array();
				...
				//echo '-------------------------------------- 3 ----------------------------------';			
				// Cuando se produce una solicitud regular es decir no ajax. Trabaja sobre todos los selects definidos.
